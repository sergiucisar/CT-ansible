...
CREATE_BROKER_COMMAND = "{0}/bin/artemis create --user {3} --password {4}"
...
def create_broker(artemis_home, ansible_module, name, path, user, password):
    ...
    changed = False
    cmd = CREATE_BROKER_COMMAND.format(
        artemis_home, path, name, user, password)
    out = ""
    err = ""
...
def main():
    ...
    changed, cmd, out, err = create_broker(
        artemis_home, ansible_module, name, path, user, password)

    ansible_module.exit_json(changed=changed, cmd=cmd,
                             name=name, stdout=out, stderr=err)
